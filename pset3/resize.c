...
int main(int argc, char *argv[])
{
    ...
    //changing values of BITMAPINFOHEADER
    bi.biWidth *= f;
    bi.biHeight *= f;
    ...
    //changing values of BITMAPFILEHEADER
    ...
    for (int i = 0, biHeight = -1*bi.biHeight*f; i < biHeight; i++)
    {
        ...
        if(counter<f){
            printf("True\n");
            fseek(inptr, -1*((bi.biWidth*3)+originalPadding), SEEK_CUR);
            printf("%li\n%i\n", ftell(inptr), counter);
        }else{
            printf("False\n");
            counter = 0;
